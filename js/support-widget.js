...
(function () {
  ...
  function injectButton(link) {
    ...
    /* The WhatsApp glyph. This button has always opened a wa.me link and its
       aria-label has always said so, but the icon was a generic pair of chat
       bubbles — so the one visual cue to where the button goes was the only
       part not saying WhatsApp. Now it matches.

       A hand-inlined path is the right call here specifically because this is
       a brand mark, not a UI icon: it has one correct shape, it is the same
       vector the footer's YouTube, Facebook, Instagram and LinkedIn marks are
       inlined as, and there is no icon package in this project to pull it
       from. Filled rather than stroked, which is how the mark is drawn. */
    a.innerHTML =
